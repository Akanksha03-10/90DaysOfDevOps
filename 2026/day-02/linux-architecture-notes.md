...
Kernel: The core, monolithic component that interacts directly with hardware, managing memory, CPU, and device drivers.

Shell: The interface between the user and the kernel. It interprets commands typed by the user and passes them to the kernel. Common shells include Bash and Zsh.

System Libraries: Specialized programs (like glibc) that allow application programs to access kernel features without requiring the kernel's code access rights.

System Utilities: Software that performs individual, specialized tasks (e.g., managing files, changing settings).

Hardware Layer: Physical components, including the CPU, memory, and devices (mouse, keyboard, storage).

File System: The organizational structure for storing and accessing data on storage devices.